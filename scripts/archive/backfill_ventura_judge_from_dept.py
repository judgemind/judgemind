#!/usr/bin/env python3
"""Backfill Ventura County judge names using the department-to-judge mapping.

Many Ventura County rulings have a department set but no judge_id, because
the ruling text does not contain judge names — it only shows the department
number (e.g., "J6", "42", "41").

The Ventura Superior Court website publishes judicial assignment pages that
map courtrooms (departments) to judges.  This script uses those mappings to
backfill missing judge_id values.

For each Ventura ruling with a department but no judge_id, this script:
...
Usage:
    scripts/with-secret.sh \\
        ...
        -- packages/scraper-framework/.venv/bin/python3 scripts/backfill_ventura_judge_from_dept.py

    # Dry run (no DB writes):
    scripts/with-secret.sh \\
        -e DATABASE_URL=judgemind/dev/db/connection:.url \\
        -- packages/scraper-framework/.venv/bin/python3 scripts/backfill_ventura_judge_from_dept.py --dry-run
...
from courts.ca.ventura_dept_judges import (  # noqa: E402
...
# Fetch Ventura County rulings that have a department but no judge_id.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Backfill Ventura County judge names from department-to-judge mapping.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    # Fetch the live dept-to-judge mapping from all three courthouse pages
    logger.info("Fetching Ventura department-to-judge mapping...")
