...
class DispatcherDaemon:
    ...
    **Phase 3B (#2787)** adds the post-PR transitions via
    ...
    def __init__(self, cfg: DaemonConfig, logger: logging.Logger):
        ...
        # Phase 3A: within-tick handoff between phase helpers was
        # previously done via ``_agent_plan_output``, ``_agent_ralph_output``,
        # ``_agent_summary_output``, and ``_agent_unmet_criteria``.
        # Replaced in #2975: all spawn sites now read from
        # ``dispatcher.phase_outputs`` via :meth:`_fetch_phase_output`
        # so the DB is the single source of truth regardless of daemon
        # restart or worktree wipe.
    ...
    def _materialize_phase_output(
        self,
        worktree: Path,
        phase: str,
        output_json: dict[str, Any],
    ) -> None:
        """Write *output_json* to ``{worktree}/tmp/dispatcher-output/<phase>.json``.

        Ensures the file exists on disk regardless of whether the output was
        freshly generated by a subprocess or fetched from
        ``dispatcher.phase_outputs`` (boot-recovery / daemon-restart path).
        Any downstream code that reads the file directly — scripts, admin
        tooling — sees an identical structure in both cases (#2937, #2975).
        """
    ...
    def _materialize_plan_output(
        ...
        """Thin wrapper around :meth:`_materialize_phase_output` for the plan phase.

        Kept for backward compatibility with the plan-reuse short-circuit
        path and existing call sites/tests (#2937).
        """
        self._materialize_phase_output(worktree, "plan", plan_output)
    ...
    def _run_ralph_phase(
        ...
        # Fetch plan output from DB — unconditional read so this path
        # works correctly after a daemon restart where the in-memory
        # cache was lost (#2975).
        plan_output = self._fetch_phase_output(agent_id, "plan") or {}
        # Materialize plan.json so downstream scripts and admin tooling
        # see the file regardless of how this invocation was reached.
        self._materialize_phase_output(worktree, "plan", plan_output)
        ralph_input = {
            "agent_id": agent_id,
            "issue_number": issue_number,
    ...
    def _run_summary_phase(
        ...
        # Fetch ralph and plan outputs from DB — unconditional reads so
        # this path works after a daemon restart (#2975).
        ralph_output = self._fetch_phase_output(agent_id, "ralph") or {}
        plan_output = self._fetch_phase_output(agent_id, "plan") or {}
        # Materialize both files so downstream scripts / admin tooling
        # see the disk artifacts regardless of how this invocation was reached.
        self._materialize_phase_output(worktree, "plan", plan_output)
        self._materialize_phase_output(worktree, "ralph", ralph_output)
        ...
        if unmet:
            ...
            # unmet_criteria is derived from the persisted summary output
            # in _push_and_open_pr via _fetch_phase_output — no local stash
            # needed (#2975).
    ...
    def _push_and_open_pr(
        ...
        **needs_review branch (#2856):** when ``unmet_criteria`` from
        the persisted summary output is non-empty, the PR is opened as
        a DRAFT with an ``⚠️ Unmet acceptance criteria`` section appended
        to the body, an issue comment is posted linking the draft +
        listing the unmet criteria, and the agent terminates as
        ...
        # Fetch summary output from DB — unconditional read so this path
        # works after a daemon restart (#2975).
        summary_output = self._fetch_phase_output(agent_id, "summary") or {}
        # Materialize summary.json so admin tooling sees the file on disk.
        self._materialize_phase_output(worktree, "summary", summary_output)
        unmet_raw = summary_output.get("unmet_criteria") or []
        ...
        unmet_criteria = [str(u) for u in unmet_raw]
    ...
    def _run_fix_ci(self, agent: dict[str, Any], pr_status: dict[str, Any]) -> None:
        ...
        # Fetch plan output from DB — unconditional read so this path
        # works after a daemon restart (#2975). The plan's change_type
        # is needed by the fix-ci skill for routing (#2966).
        plan_output = self._fetch_phase_output(agent_id, "plan") or {}
        ralph_output = self._fetch_phase_output(agent_id, "ralph") or {}
        summary_output = self._fetch_phase_output(agent_id, "summary") or {}
        # Materialize output files so admin tooling and the subprocess
        # see consistent disk artifacts regardless of daemon restart.
        self._materialize_phase_output(worktree, "plan", plan_output)
        self._materialize_phase_output(worktree, "ralph", ralph_output)
        self._materialize_phase_output(worktree, "summary", summary_output)
        ...
        fix_ci_input = {
            ...
            "change_type": plan_output.get("change_type", ""),
        }
    ...
    def _run_verify_and_complete(
        ...
        # Fetch plan and summary outputs from DB — unconditional reads so
        # this path works after a daemon restart (#2975).  The plan's
        # ``plan_text`` and ``scope_check`` are threaded to the verify
        # skill (matching the agent-runner-entrypoint.sh shim).  The
        # summary's ``deferred_acs`` (#3010) are normalised below.
        plan_persisted = self._fetch_phase_output(agent_id, "plan") or {}
        summary_persisted = self._fetch_phase_output(agent_id, "summary") or {}
        # Materialize output files so admin tooling sees consistent disk
        # artifacts regardless of daemon restart.
        self._materialize_phase_output(worktree, "plan", plan_persisted)
        self._materialize_phase_output(worktree, "summary", summary_persisted)

        # Prefer plan-output's AC list (authoritative at claim time); fall
        # back to extracting from the issue body.
        plan_acs = plan_persisted.get("acceptance_criteria") or []
        if plan_acs:
            acceptance_criteria = plan_acs

        # Issue #3010 — thread ``deferred_acs`` from summary to the verify
        # skill.  The verify skill runs the deferred ACs first (summary
        # skipped them pre-merge) and labels each as
        # "deferred (marker|heuristic) → pass|fail" in the evidence comment.
        # Absent/empty on pre-#3010 agents and on no-deploy types — verify
        # treats the universe as "every AC, no labeling" in that case (see
        # task-v2-verify SKILL.md).
        ...
        verify_input = {
            ...
            "worktree_path": str(worktree),
            "repo_root": str(worktree),
            "plan_text": plan_persisted.get("plan_text", ""),
            "scope_check": plan_persisted.get("scope_check", []) or [],
    ...
    def _run_retro_phase(self, agent: dict[str, Any]) -> None:
        ...
        # Materialize prior phase output files from DB so the retro
        # subprocess and any downstream scripts see consistent disk
        # artifacts regardless of daemon restart (#2975).
        for _phase in ("plan", "verify", "summary"):
            _output = self._fetch_phase_output(agent_id, _phase) or {}
            if _output:
                self._materialize_phase_output(worktree, _phase, _output)
