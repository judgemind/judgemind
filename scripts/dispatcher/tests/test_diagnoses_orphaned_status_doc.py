...
class TestMigration60Exists:
    def test_file_exists(self) -> None:
        assert MIGRATION_60.is_file(), f"missing: {MIGRATION_60}"

    def test_has_up_and_down_sections(self) -> None:
        text = MIGRATION_60.read_text()
        assert "-- Up Migration" in text
        assert "-- Down Migration" in text


class TestMigration60UpSection:
...
class TestMigration60DownSection:
