...
Two columns added in migration 50:
...
MIGRATION_50 = (
    REPO_ROOT
    / "packages/api/migrations/50_dispatcher-diagnoses-actions-and-directive.sql"
)
...
class TestMigration50Exists:
    def test_file_exists(self) -> None:
        assert MIGRATION_50.is_file(), f"missing: {MIGRATION_50}"

    def test_has_up_and_down_sections(self) -> None:
        text = MIGRATION_50.read_text()
...
class TestMigrationNumbering:
    def test_no_collision_with_existing(self) -> None:
        ...
        files = sorted(migrations_dir.glob("50_*.sql"))
        assert len(files) == 1, f"unexpected 50_* migrations: {files}"
        # The new migration is the only one with the 50 prefix.
        assert files[0].name.startswith("50_dispatcher-")

    def test_no_higher_numbered_migration_exists(self) -> None:
        # 50 is intended as the new tip; 51+ would mean another migration
        ...
        # Top file is the 50 migration.
        assert higher, "no migrations found"
        top_num = int(higher[0].name.split("_", 1)[0])
        assert top_num <= 50, (
            f"a higher-numbered migration than 50 exists ({higher[0].name}); "
