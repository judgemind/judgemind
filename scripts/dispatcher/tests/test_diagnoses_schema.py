...
Two columns added in migration 49:
...
MIGRATION_49 = (
    REPO_ROOT
    / "packages/api/migrations/49_dispatcher-diagnoses-actions-and-directive.sql"
)
...
class TestMigration49Exists:
    def test_file_exists(self) -> None:
        assert MIGRATION_49.is_file(), f"missing: {MIGRATION_49}"

    def test_has_up_and_down_sections(self) -> None:
        text = MIGRATION_49.read_text()
...
class TestMigrationNumbering:
    def test_no_collision_with_existing(self) -> None:
        ...
        files = sorted(migrations_dir.glob("49_*.sql"))
        assert len(files) == 1, f"unexpected 49_* migrations: {files}"
        # The new migration is the only one with the 49 prefix.
        assert files[0].name.startswith("49_dispatcher-")

    def test_no_higher_numbered_migration_exists(self) -> None:
        # 49 is intended as the new tip; 50+ would mean another migration
        ...
        # Top file is the 49 migration.
        assert higher, "no migrations found"
        top_num = int(higher[0].name.split("_", 1)[0])
        assert top_num <= 49, (
            f"a higher-numbered migration than 49 exists ({higher[0].name}); "
