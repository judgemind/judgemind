...
Two columns added in migration 48:
...
MIGRATION_48 = (
    REPO_ROOT
    / "packages/api/migrations/48_dispatcher-diagnoses-actions-and-directive.sql"
)
...
class TestMigration48Exists:
    def test_file_exists(self) -> None:
        assert MIGRATION_48.is_file(), f"missing: {MIGRATION_48}"

    def test_has_up_and_down_sections(self) -> None:
        text = MIGRATION_48.read_text()
...
class TestMigrationNumbering:
    def test_no_collision_with_existing(self) -> None:
        ...
        files = sorted(migrations_dir.glob("48_*.sql"))
        assert len(files) == 1, f"unexpected 48_* migrations: {files}"
        # The new migration is the only one with the 48 prefix.
        assert files[0].name.startswith("48_dispatcher-")

    def test_no_higher_numbered_migration_exists(self) -> None:
        # 48 is intended as the new tip; 49+ would mean another migration
        ...
        # Top file is the 48 migration.
        assert higher, "no migrations found"
        top_num = int(higher[0].name.split("_", 1)[0])
        assert top_num <= 48, (
            f"a higher-numbered migration than 48 exists ({higher[0].name}); "
