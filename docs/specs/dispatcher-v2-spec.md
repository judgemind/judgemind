...
- Hook failure signals (`no_commit_on_exit`, `cwd_drift`, etc.) — **hooks write directly to `dispatcher.failures` via `scripts/dispatcher/emit_failure.py`** (thin psycopg2 wrapper, ~15 lines). Hooks run inside the `/task-v2-*` subprocess, which inherits `DATABASE_URL` from the container env and is in the same VPC as the DB. No marker-file indirection. Insert is wrapped in try/except + log: hook failure must never block the subprocess, and DB transient errors fall through to PR-vs-GH reconciliation (Risk 1) which still catches the crashed-agent case.
...
3. **Poll queue.** `gh issue list --label agent/ready --state open`. Cross-reference `dispatcher.agents` to exclude issues already in-flight.
...
| `/task-v2-ralph` | plan from above, worktree path | SHIP verdict + implementation diff in git | ~45-90 min (multi-invocation internally, same as today) |
...
| **Gemini CLI** (`@google/gemini-cli`, [repo](https://github.com/google-gemini/gemini-cli)) | `gemini -p '<prompt>' --output-format stream-json` | yes | Nearly 1:1 with Claude Code — `BeforeTool`/`AfterTool`/`SessionStart`/`SessionEnd`, same `settings.json`-style shell-exec with stdin/stdout JSON, exit code `2` blocks | `.gemini/skills/` **or** `.agents/skills/` (cross-tool standard) | yes, native | Exit codes 0/1/42/53 (53 = turn-limit — *better* granularity than `claude -p`'s `1`-for-everything). Auth via OAuth free tier (1000 rpd on Gemini 3 Pro), AI Studio key, or Vertex. Tool names differ (`read_file`/`write_file`/`run_shell_command`) so hook matchers need a rewrite layer. |
...
- **Claude Code and Gemini CLI — near-full parity.** Both support shell-exec hooks with matcher regex, stdin/stdout JSON contracts, and exit-code-based blocking. `SessionStart`/`SessionEnd` map to our `SubagentStop` use case. `emit_failure.py` works unchanged; we just register the hooks in both `settings.json` files and account for different tool names in the matchers.
...
| `rate_limit_529` | 1 | Scheduler, reading stderr of `claude -p` subprocess | Regex on Anthropic 529 error body → exponential backoff retry |
...
| `subprocess_crash` | 1 | Scheduler | Exit code not in known set → retry with fresh worktree |
...
All hooks write **directly to Postgres** via a shared helper `scripts/dispatcher/emit_failure.py`. This helper is a thin psycopg2 wrapper (~15 lines): opens a connection using `DATABASE_URL` from the env, inserts one row into `dispatcher.failures`, closes. The insert is wrapped in try/except — any DB error is logged to stderr and swallowed, never propagated up. Hooks execute inside the `/task` subprocess, which shares the daemon's VPC and inherits the same `DATABASE_URL` env var, so network/auth are free.
...
- Secrets (via Secrets Manager → env):
  - `ANTHROPIC_API_KEY`
  - `GITHUB_TOKEN` (scoped token, not the agent account)
...
1. **Per-phase `claude -p` context budget verification** — the per-phase split (§6a) is our response to the no-auto-compact constraint. Each `/task-v2-*` invocation should stay well within a single context window (most phases are 10-30 min; `/task-v2-ralph` is the long tail at 45-90 min). **Action:** run each `/task-v2-*` skill end-to-end as `claude -p` in a container against a representative issue, measure (a) peak context usage, (b) whether context pressure manifests as a clean error or silent degradation. If `/task-v2-ralph` is the only one that flirts with limits, its internal worker/reviewer loop is already subdividable — we would sub-split it into `/task-v2-ralph-worker` + `/task-v2-ralph-review`, each one iteration.
