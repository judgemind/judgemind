...
**Automated lint (#4549):** `scripts/check-issue-verify-test-filename.py --issue <N>` (or `--body-file <path>` for a draft) parses the issue body and flags `Verify:` lines that name `scripts/tests/_*.sh` paths or the historical pre-#4545 `*-test.sh` antipattern. Exit 0 = clean; exit 1 = at least one offending filename (with a per-line diagnostic naming the bad path and the recommended `test_<thing>.sh` / `test__<thing>.sh` replacement); exit 2 = `gh` error. The `.github/workflows/issue-verify-test-filename.yml` workflow runs the same check on `issues: [opened, edited]` and strips `agent/ready` if it fails, so authors get the signal at issue-creation time. Sibling pattern to `check-issue-verify-sql.py` (#4358).
