...
| `dispatcher-polled-cost-` | `api-service` | log metric filter on `/ecs/judgemind-api-${env}` matching `{ $.msg = "graphql.cost" }`, namespace `Judgemind/API`, metric `PolledQueryCost` | Cockpit's polled `dispatcherState` query cost > 900 (cap 1000) in 3 of last 5 minutes — pre-cap early warning, see #4110. Issue #4101 collapsed the previous `graphql.cost.breakdown` log line into `graphql.cost` so the alarm and the per-field `breakdown` map come from one log event. |
...
| `judgemind-scraper-emit-failure-` | `compute` | log metric filter on `/ecs/judgemind-scraper-${env}` matching `"Failed to emit document.captured event"`, namespace `judgemind/scraper`, metric `ScraperRedisEmitFailures` | Scraper logged > 1 emit failure in any 1-minute window — typically Redis OOM (cache memory > 95%) or Redis network partition. Producer-side ingestion failure that broke 2026-05-04 → 2026-05-09 (#4470, #4475) |
| `judgemind-${env}-cache-memory-warn` (search key: `judgemind-cache-memory-warn`) | `cache` | `AWS/ElastiCache` `DatabaseMemoryUsagePercentage` on `CacheClusterId = judgemind-${env}` | ElastiCache memory > 80% for 5 consecutive minutes — leading-indicator alert, bump node_type before the OOM hits (#4475) |
| `judgemind-${env}-cache-memory-critical` (search key: `judgemind-cache-memory-critical`) | `cache` | `AWS/ElastiCache` `DatabaseMemoryUsagePercentage` on `CacheClusterId = judgemind-${env}` | ElastiCache memory > 95% for 1 minute — critical, Redis is at imminent risk of OOM-rejecting XADD writes (#4470, #4475) |
...
| `judgemind-dispatcher-scheduler-tick-cadence-slow-` | `dispatcher-daemon` | log metric filter on `daemon.scheduler_tick` events extracting `inter_tick_seconds` into `Judgemind/Dispatcher` metric `TickCadenceSeconds`; companion log metric filter `tick-cadence-slip` counts `daemon.tick_cadence_slip` WARNINGs into `TickCadenceSlipCount` for triage queries (no alarm on the count) | Dispatcher scheduler-tick cadence p95 > `scheduler_tick_cadence_threshold_seconds` (default 90s) over `scheduler_tick_cadence_window_seconds` (default 300s / 5min) — the loop is silently slow, distinct from the #3097 stall watchdog which exits the daemon at 120s. See #2854. |
