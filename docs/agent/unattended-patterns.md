...
- **Never bare `git stash pop` / `git stash apply` — the stash list is shared across worktrees:** `git stash` stores refs in `$GIT_DIR/refs/stash`, which is a single per-clone stack. Every worktree in the clone sees the same stash list, so a bare `git stash pop` in worktree A can silently apply a stash that was created by worktree B (or by an agent that was cleaned up weeks ago). The current worktree's edits get overwritten, and the other agent's WIP lands in your diff — where `git add -A` or similar broad staging can push it into your next commit. Observed during #2746 (see #2749). **Safer patterns:**
  1. **Pop by explicit ref.** Run `git stash list --pretty=format:'%gd %s'` first, confirm the `stash@{0}` subject contains your current branch name, then pop with the explicit ref: `git stash pop stash@{0}`. A stash commit SHA is also an explicit ref — capture it with `git stash list --format='%H %gs'` and restore with `git stash apply <sha>` (#4683). Never trust `stash@{0}` without checking its subject.
  ...
  The preflight hook (`.claude/hooks/preflight-bash.sh` Check 12) blocks bare `git stash pop` and `git stash apply` — every pop/apply in the command needs its own `stash@{N}` or SHA argument. `git stash list`, `git stash show`, `git stash push`, `git stash drop`, and `git stash clear` are all allowed.
