...
- **NEVER** run bare `git stash pop` or `git stash apply` — `.git/refs/stash` is shared across every worktree in the clone, so a bare pop can silently apply another agent's or another worktree's stash (reverting your edits and dumping their WIP into your worktree; observed in #2749). Always pop by explicit ref: run `git stash list` first, confirm `stash@{0}`'s subject contains your current branch, then `git stash pop stash@{0}`. Better yet, avoid stash entirely — use a throwaway commit (`git commit -am "WIP"`, do the thing, `git reset --soft HEAD~1`) which has no shared global state. The preflight hook blocks bare pop/apply.
...
The PreToolUse hook (`.claude/hooks/preflight-bash.sh`) and `scripts/preflight.sh` automatically enforce the Critical Rules above. The hook blocks `$(`, `<<EOF`, `python -c`, `git push` to main, `git worktree add` inside worktrees, cross-worktree writes via Bash (cp/mv/tar/redirection into the main repo checkout from a worktree subagent — see #2455), and bare `git stash pop` / `git stash apply` (see #2749 — the stash list is per-clone, not per-worktree, so bare pops can apply another worktree's stash). A separate PreToolUse hook (`.claude/hooks/agent-worktree-guard.sh`) blocks Agent tool calls with `isolation: "worktree"` when cwd has drifted into an existing worktree, preventing nested worktree creation. Another PreToolUse hook (`.claude/hooks/worktree-write-guard.sh`) blocks Edit/Write on paths inside the main repo checkout when the agent is running inside a worktree, preventing silent writes to the main repo that bypass the PR workflow (#2440). Runtime checks:

**Operator-interactive vs. Fargate dispatcher scope.** The rules above all apply in **operator-interactive sessions** (developer laptops). Most of them exist to prevent the interactive Claude CLI from stalling on permission prompts — a problem that does not exist in the non-interactive Fargate dispatcher container, where subagents run with `--allow-dangerously-skip-permissions` (the `--allow-` variant is used because the plain `--dangerously-skip-permissions` refuses to run under root/sudo and the Fargate container has no `USER` line). Inside the Fargate image (`Dockerfile.dispatcher`) the preflight hook is file-swapped at worktree creation time to a narrowed variant (`scripts/preflight-bash-fargate.sh`) that only enforces the 4 safety-critical rules: `git push` to main, `git worktree add` inside a worktree, cross-worktree writes into the main repo, and bare `git stash pop` / `git stash apply`. The 6 interactive-prompt-prevention rules (`$(...)`, heredocs, inline `python -c`, quoted strings + `&&`/`;`, `bash script/` prefix, empty-quotes bypass) are allowed in-container because they are safe there. Operator laptops keep the full ruleset — this change is scoped to the Fargate image via `DISPATCHER_FARGATE_HOOKS_DIR` + per-worktree `git update-index --skip-worktree`. See issues #2982 and #2986.
...
- **Claim interlock (`status/in-progress` label, label-only).** On claim, `/task` adds the `status/in-progress` GitHub label **before** removing `agent/ready` so a racing daemon tick sees the new label during the ~100ms propagation window. The daemon's queue-scan filter drops any issue carrying `status/in-progress`, and the daemon's `_atomic_claim` re-reads the label set at claim time as a final fence. On terminal (PR merged, verification passed, blocker, investigation closed, STUCK exit), `/task` removes the label. The daemon still writes its own `dispatcher.agents` row for daemon↔daemon atomic collision detection via the partial UNIQUE INDEX `idx_dispatcher_agents_active_issue`, but `/task` subagents no longer touch that table. The residual race window — both operator-add and daemon-scan completing within the same ~100ms — is accepted (worst case: two PRs on the same issue, a human notices and closes one). Issue #2927 replaced the prior DB-row + label interlock (#2866) with this label-only flow. See `.claude/skills/task/SKILL.md` §2a + A.7 + B.2 for the exact commands.
