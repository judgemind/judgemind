...
When the issue's title or body classifies as **audit-class** — any of `audit`, `investigate` / `investigation`, `refactor` / `refactoring`, `migrate` / `migration`, `extend` / `extension`, `tighten`, `harden`, `additional` (word-boundary-anchored, case-insensitive) — the threshold tightens to **≥2 target-context overlaps AND every overlap is ADDED** (#4223). Audit / investigation / refactor issues are by intent asking for *more* work on existing files, so a single added or any modified overlap is too weak a signal — the canonical FP is "prior unrelated PR touched the file the audit cites." Date-ordering (#4353) closes the easy case; this tightening closes the residual where the unrelated PR merged AFTER the issue was filed. The classifier helper lives at `scripts/_check_shipped_pr_classify_issue.py` and is called by the bash wrapper before invoking the overlap helper.
