...
>    - Run from the repo root: `scripts/check-diff-coverage.sh <pkg> --skip-tests`. It runs diff-cover with the same flags as CI: 90% floor, compared against the merge-base with `origin/main` (`--diff-range-notation='...'`), so commits that landed on main after you branched are not counted as your lines (#4764).
...
>    - For TypeScript packages, run the same script: `scripts/check-diff-coverage.sh <pkg> --skip-tests` (after `npm test` has written `coverage/lcov.info`).
