...
3. "Ralph iteration 1 — Claude review" (activeForm: "Claude reviewing iteration 1")
...
- If **COMPLETE**: Continue to the Gemini review phase.

### 2b — Gemini review phase (cross-model)

Write status: `phase: ralph-reviewer (iteration N)`, `summary: Gemini cross-model review for iteration N`.

Run the Gemini review script, which sends the diff and task context to Gemini 2.5 Pro for an independent review:
...
The script:
1. Generates `diff.txt` and `changed_files.txt` from the worktree's git state
2. Injects the Google API key from Secrets Manager
3. Calls Gemini 2.5 Pro with the review prompt
4. Writes the verdict to `{worktree}/tmp/ralph/gemini-review-result.txt` (SHIP, REVISE, or SKIPPED)
5. Writes detailed feedback to `{worktree}/tmp/ralph/gemini-feedback.md`
6. Appends a structured review record to `{worktree}/tmp/ralph/review-log.jsonl` (automatic — no action needed)

After the script completes, read `{worktree}/tmp/ralph/gemini-review-result.txt`:

- If **SKIPPED**: Gemini review was unavailable (no API key, API error). Continue to Claude review without Gemini feedback. This is not a failure — the loop degrades gracefully.
- If **SHIP** or **REVISE**: Continue to Claude review. The Claude reviewer will see Gemini's feedback and factor it into its own review.

### 2c — Claude review phase

Write status: `phase: ralph-reviewer (iteration N)`, `summary: Claude reviewer evaluating iteration N`.

Read the Gemini feedback file to include in the Claude reviewer's prompt:

Read `{worktree}/tmp/ralph/gemini-feedback.md` and `{worktree}/tmp/ralph/gemini-review-result.txt`.
...
>
> **Gemini cross-model review has already been completed.** Read the Gemini reviewer's feedback at `{worktree}/tmp/ralph/gemini-feedback.md` — consider their findings in your review. You may agree or disagree with Gemini's assessment. If Gemini flagged issues, verify whether they are valid. If Gemini said SHIP, still do your own thorough review.
>
> 1. Read the task requirements: `{worktree}/tmp/ralph/task.md`
> 2. Read the Gemini review feedback: `{worktree}/tmp/ralph/gemini-feedback.md`
...
After the reviewer subagent completes, read `{worktree}/tmp/ralph/review-result.txt` and `{worktree}/tmp/ralph/gemini-review-result.txt`.
...
**Both reviewers must agree to SHIP:**

- If Claude says **SHIP** AND (Gemini says **SHIP** or **SKIPPED**): The loop is done. Continue to Step 3.
- If EITHER reviewer says **REVISE**: Increment iteration. If `iteration > max_iterations`, stop the loop and comment on the issue that the ralph loop hit its max iterations — add `status/blocked` and return with failure. Otherwise:
  - If Gemini said REVISE but Claude said SHIP, append Gemini's feedback to `{worktree}/tmp/ralph/feedback.md` so the worker addresses it in the next iteration.
  - If Claude said REVISE (regardless of Gemini), Claude's feedback is already in `feedback.md`.
  - If both said REVISE, consolidate both sets of feedback into `feedback.md`.
  - Create new todos for the next iteration ("Ralph iteration N — worker", "Ralph iteration N — Gemini review", "Ralph iteration N — Claude review"), then return to 2a.
