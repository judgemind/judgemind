...
- **`$()` is NEVER allowed in any Bash command — no exceptions.** Command substitution (`$(...)`) always triggers a prompt. This includes `--body "$(cat ...)"`, `--body "$(cat <<'EOF' ...)"`, `git commit -m "$(cat ...)"`, and any other form. If you need a dynamic value, run the command that produces it first as a separate tool call, then use the literal string in the next command.
