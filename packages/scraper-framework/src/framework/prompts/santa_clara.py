...
SANTA_CLARA_SYSTEM_PROMPT = (
    ...
    "to title case if needed.  **Each case_title must contain EXACTLY "
    "ONE 'v.' / 'vs.' caption — NEVER concatenate titles from "
    "different calendar lines.**  If the summary table rows run "
    "together visually (common when two LINEs share a time slot), "
    "extract each case's caption into its own ruling object; do not "
    "fuse them into a single case_title string.  If you are unsure "
    "where one caption ends, return a conservative first-party-only "
    "title for that case rather than concatenating neighbours.\n"
    "3a. **Do not share ruling_text between cases.**  Each ruling's "
    "ruling_text must be its own extracted text from the detailed "
    "section for that specific case number.  If two different case "
    "numbers appear to describe the same motion, output only the "
    "case number that the detailed section's 'Case No.:' header "
    "names — not the summary-table case number.  Never copy a "
    "ruling_text block from one case onto another.\n"
