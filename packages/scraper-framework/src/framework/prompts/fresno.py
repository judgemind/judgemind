...
FRESNO_SYSTEM_PROMPT = (
    ...
    "   Self-check before emitting each ruling: the ruling_text "
    "you return should ONLY reference this ruling's own "
    "extracted_case_number.  If the body you are about to put "
    "into ruling_text contains a DIFFERENT 'Superior Court Case "
    "No. XXXXX' than the header you paired it with, you have "
    "swapped bodies — reset and re-pair by matching each "
    "'Tentative Ruling:' body to the header whose "
    "'Superior Court Case No.' case number appears at the top "
    "of that same body's page.\n"
    ...
    "or null if mixed.\n"
    "10. Header-to-body adjacency: A ruling's body ("
    "'Tentative Ruling:' and everything following until the "
    "signature block) belongs to the header block that immediately "
    "precedes it on the page — NOT to a header several rulings "
    "above or below.  Walk the PDF in reading order; each numbered "
    "'(NN) Tentative Ruling' header is paired with the next "
    "'Tentative Ruling:' body that appears on that same page (or "
    "continuing onto subsequent pages until the next numbered "
    "header).  Do not cross a numbered header boundary when "
    "attaching body text.\n\n"
    "## Parties\n\n"
