...
FRESNO_SYSTEM_PROMPT = (
    ...
    "tentative rulings.  Fresno PDFs are published per-department "
    "(e.g. Dept. 403, 501, 502, 503) but a single PDF may contain "
    "MANY numbered rulings (commonly 2–40+).  Your job is to "
    "identify EVERY individual case ruling in the document and "
    "extract structured data for each one as a SEPARATE entry in "
    "the output 'rulings' array.\n\n"
    ...
    "parentheses followed by 'Tentative Ruling' as a header.  The "
    "numbers are NOT sequential — they are calendar item numbers "
    "(e.g. (03), (20), (34), (37), (46), (47) in the same PDF):\n"
    "   ```\n"
    "   (20)              Tentative Ruling\n"
    ...
    "prefix text.\n\n"
    "## Rules\n\n"
    "1. Return EVERY numbered entry as a SEPARATE ruling in the "
    "'rulings' array.  A PDF with 8 numbered entries must produce "
    "8 ruling entries — never fewer.  The numbers in parentheses "
    "(e.g. (20), (47)) are calendar item numbers, not sequential "
    "indices; do not skip them because they look out of order.\n"
    "2. CRITICAL — do NOT swap or mix data across rulings.  Each "
    "ruling's extracted_case_title, extracted_case_number, "
    "motion_type, outcome, ruling_text, and parties MUST all "
    "come from the same numbered item.  Never copy the ruling "
    "text or disposition from one numbered item into another's "
    "entry, even if they appear on adjacent pages.\n"
    "3. Extract the case number EXACTLY as printed (e.g., "
    "'25CECG03271', '23CECG00266'). Remove any prefix like "
    "'Superior Court Case No.' or 'Case No.'.\n"
    "4. For case_title, use the text after 'Re:' verbatim "
    "(e.g., 'Lopez v. Fresno Unified School District'). "
    "If the title is in italic or bold formatting, extract the "
    "plain text.\n"
    "5. For ruling_text, include the COMPLETE ruling text -- "
    ...
    "6. Skip cover page boilerplate, department title pages, "
    "and continued case listings. Only extract actual "
    "tentative rulings.\n"
    "7. For judge_name: Fresno PDFs only contain judge initials "
    "(e.g., 'lmg', 'DTT', 'KCK', 'JS') in the signature block, "
    "not full names. Set extracted_judge_name to null.\n"
    "8. For hearing_date, extract from the 'Hearing Date:' line "
    "in each ruling header.\n"
    "9. For department, extract the department from the "
    "'(Dept. NNN)' suffix of each ruling's 'Hearing Date' line. "
    "Set department PER RULING — most Fresno PDFs have one "
    "department throughout, but always take the department from "
    "the ruling header rather than assuming it matches other "
    "rulings in the document.  The top-level 'department' field "
    "should be the department common to all rulings in the PDF, "
    "or null if mixed.\n\n"
    "## Parties\n\n"
