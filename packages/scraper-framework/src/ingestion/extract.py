...
# ---------------------------------------------------------------------------
# Case title extraction — caption block strategy (#1405)
# ---------------------------------------------------------------------------
# Formal "Plaintiff vs. Defendant" caption blocks found in LA rulings.
# These are the most reliable source of case titles because the party names
# appear in a structured format.  Migrated from scripts/backfill_case_titles.py
# to consolidate all extraction logic in one place.
...
_CAPTION_DESCRIPTOR_RE = re.compile(
...
def _clean_party_name(raw: str) -> str:
    ...
    name = _CAPTION_DESCRIPTOR_RE.sub("", name).strip().rstrip(",").strip()
    # Strip "et al." suffix
...
# ---------------------------------------------------------------------------
# Case title extraction — Case Name field strategy (#1405)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Case title extraction — MOVING/RESPONDING PARTY strategy (#1405)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Case title extraction — regex patterns (inline "v." matching)
# ---------------------------------------------------------------------------
...
_CASE_TITLE_PATTERNS: list[re.Pattern[str]] = [
    # NOTE: "Case Name:" / "Case Title:" is handled by _extract_from_case_name_field
    # (Strategy 2, with "v." validation) and intentionally excluded from this list
    # to avoid matching non-party content like "Motion for Summary Judgment" (#1405).
    #
...
def extract_case_title(ruling_text: str) -> str | None:
    ...
    4. Regex "v." patterns (inline, Riverside-style, generic) — broad fallback
    5. "In re:" / "In the Matter of" / "Petition of" — non-adversarial cases
    ...
    # Strategy 4: Regex "v." patterns — broad fallback
