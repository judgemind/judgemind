...
COMMENT ON FUNCTION dispatcher.issue_has_active_agent(p_issue_number integer) IS 'Returns TRUE when dispatcher.agents has any row for this issue with status IN (''running'', ''retrying'', ''needs_review''), OR with status = ''succeeded'' AND merged_at IS NULL (PR not yet detected as merged). A succeeded row with merged_at IS NOT NULL no longer blocks re-claim — the housekeeping tick closes the stale issue via _cleanup_stale_succeeded_rows. Issue #3738 (prior definition: Issue #3001).';
